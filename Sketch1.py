...
class Sketch(CanvasBase):
    ...
    * buff_last: Buff. Last frame buffer
    ...
    def Interrupt_MouseL(self, x, y):
        ...
        # Draw a point when user inputs one point or a line when user clicks two points 
        if len(self.points_l) == 1:
            if self.debug > 0:
    ...
    def Interrupt_Keyboard(self, keycode):
        ...
        * t, T, RIGHT, DOWN: Next Test case
        ...
        if chr(keycode) in ".>":
            ...
            self.test_case_list[self.test_case_index](self.n_steps)
            print("Display Test case: ", self.test_case_index, "n_steps: ", self.n_steps)
        #anti aliasing code to work when pressing a 
    ...
    def drawLine(self, buff, p1, p2, doSmooth=True, doAA=False, doAAlevel=4):
        ...
        while True:
            ...
            # Draw the pixel
            self.drawPoint(buff, Point((x0, y0), color))
    ...
    def drawTriangle(self, buff, p1, p2, p3, doSmooth=True, doAA=False, doAAlevel=4, doTexture=False):
        ...
        points = sorted([p1, p2, p3], key=lambda p: p.coords[1])

        v1, v2, v3 = points

        # interpolate between two points
        def interpolate(p_start, p_end, y):
            t = (y - p_start.coords[1]) / (p_end.coords[1] - p_start.coords[1]) if p_start.coords[1] != p_end.coords[1] else 0
            x = int(p_start.coords[0] + t * (p_end.coords[0] - p_start.coords[0]))
            color = ColorType(
                p_start.color.r + t * (p_end.color.r - p_start.color.r),
                p_start.color.g + t * (p_end.color.g - p_start.color.g),
                p_start.color.b + t * (p_end.color.b - p_start.color.b)
            )
            return x, color

        # Drawing scanline for each y
        for y in range(v1.coords[1], v3.coords[1] + 1):
            if y < v2.coords[1]:
                x_start, c_start = interpolate(v1, v2, y)
                x_end, c_end = interpolate(v1, v3, y)
            else:
                x_start, c_start = interpolate(v2, v3, y)
                x_end, c_end = interpolate(v1, v3, y)

            # Interpolate between two points of the scanline
            for x in range(min(x_start, x_end), max(x_start, x_end) + 1):
                if doSmooth:
                    t = (x - x_start) / (x_end - x_start) if x_start != x_end else 0
                    color = ColorType(
                        c_start.r + t * (c_end.r - c_start.r),
                        c_start.g + t * (c_end.g - c_start.g),
                        c_start.b + t * (c_end.b - c_start.b)
                    )
                else:
                    color = c_start
